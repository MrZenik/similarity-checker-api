...
class CodeFingerprint:
    ...
    filter : bool, default=True
        If set to False, code will not be tokenized & filtered.
    fp : TextIO, default=None
        ...
        the report.
    skip_imports : bool
        Value for auto removing imports
    skip_punctuation : bool
        Value for skip checking punctuation
    ...
    def __init__(self, file, k, win_size, boilerplate=[], filter=True,
                 skip_imports=False, skip_punctuation=False, language=None, fp=None):
        if fp is not None:
            code = fp.read()
        else:
            with open(file, encoding="utf-8") as code_fp:
                code = code_fp.read()
        if filter:
            filtered_code, offsets = filter_code(code, file, language, skip_imports, skip_punctuation)
        else:
...
def compare_files(file1_data, file2_data):
    ...
    Parameters
    ----------
    file1_data : CodeFingerprint object of file #1.
    file2_data : CodeFingerprint object of file #2.
...
def save_to_db(code_list, teacher_id):
    ...
    for item in code_list:
        ...
        select_query = """
                    SELECT file1, file2 FROM files_pair
                    WHERE file1 = %s AND file2 = %s AND teacher_id = %s
                """
        values = (item[2], item[3], teacher_id)
        ...
        if result:
            ...
            update_query = """
                        UPDATE files_pair
                        SET code1 = %s, code2 = %s, similarity_score = %s
                        WHERE file1 = %s AND file2 = %s
                    """
            update_values = (item[4], item[5], item[0], item[2], item[3])
            cursor.execute(update_query, update_values)
        else:
            # Insert a new pair
            insert_query = """
                        INSERT INTO files_pair (file1, file2, code1, code2, similarity_score, teacher_id)
                        VALUES (%s, %s, %s, %s, %s, %s)
                    """
            insert_values = (item[2], item[3], item[4], item[5], item[0], teacher_id)
...
class CopyDetector:
    ...
    def __init__(self, config=None, test_dirs=[], ref_dirs=[],
                 ...
                 display_t=defaults.DISPLAY_THRESHOLD, teacher_id=0,
                 disable_filtering=False, remove_imports=False,
                 force_language=None, truncate=False, punctuation=False,
                 out_file="./report.html", silent=False):
    ...
    def _load_config(self, config):
        ...
        if "remove_imports" in config:
            self.remove_imports = config["remove_imports"]
    ...
    def _check_arguments(self):
        ...
        if not isinstance(self.punctuation, bool):
            raise TypeError("remove_imports must be true or false")
        if not isinstance(self.remove_imports, bool):
            raise TypeError("punctuation must be true or false")
    ...
    def _get_file_list(self, dirs, exts):
        ...
        boilerplate_dirs.
        If in directory appears .zip file it will be
        unzipped and added to all files.
        """
        file_list = []
        extracted_files = []
        for dir in dirs:
            ...
            for ext in exts:
                ...
                for file_name in files:
                    if zipfile.is_zipfile(file_name):
                        zip_dir = Path(file_name).with_suffix("")
                        with zipfile.ZipFile(file_name, "r") as zip_ref:
                            zip_ref.extractall(zip_dir)

                        extracted_files.extend(str(f) for f in zip_dir.rglob("*") if f.is_file())
                        Path(file_name).unlink()  # delete the ZIP file
                        print_warning = False
                    else:
                        if len(files) > 0:
                            print_warning = False
            ...
            if print_warning:
                logging.warning(f"No files found in {dir}")

        extracted_files = [f for f in extracted_files if not f.endswith('.zip')]
        file_list = [f for f in file_list if not f.endswith('.zip')]
        file_list = list(set(file_list + extracted_files))
        ...
        for file in self.boilerplate_files:
            try:
                fingerprint = CodeFingerprint(file, self.noise_t, 1,
                                              ...
                                              skip_imports=self.remove_imports,
                                              skip_punctuation=self.punctuation,
            ...
            if code_f not in self.file_data:
                try:
                    self.file_data[code_f] = CodeFingerprint(
                        ...
                        self.remove_imports, self.punctuation, self.force_language)
    ...
    def generate_html_report(self, output_mode="save"):
        ...
        data_dir = pkg_resources.resource_filename('report', '/')
        ...
        save_to_db(code_list, self.teacher_id)
        ...
        if output_mode == "save":
            ...
            if not self.silent:
                print(f"Report saved to {self.out_file.replace('//', '/')}")
